        ...
        for pathbuf in c.iter() {
            let res = rules
                ...
                .map(|rule| match rule.condition {
                    Includes(_) => ConditionOps::includes(&pathbuf, &rule.text, &rule.search_type),
...
impl ConditionOps {
    ...
    pub fn match_against() -> bool {
        todo!()
    }

    pub fn includes(pathbuf: &PathBuf, to_match: &String, search_type: Condition) -> bool {
        match &search_type[..] {
            "File Name" => {
                if pathbuf.is_file() {
